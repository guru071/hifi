...
export default function ShopClient({ initialProducts }: { initialProducts: ProductWithDetails[] }) {
  ...
  const initialPrices = products.map(p => Number(p.base_price));
  const absoluteMaxPrice = initialPrices.length ? Math.ceil(Math.max(...initialPrices)) : 0;
  ...
  const anyFilter =
    search || selectedCategories.length > 0 || selectedSizes.length > 0 || maxPrice < absoluteMaxPrice || sort !== "newest";
      ...
      {absoluteMaxPrice > 0 && (
        <div className={styles.filterGroup}>
          ...
          <div className={styles.priceRangeLabels}>
            <span>{inr(0)}</span>
            <span>{inr(absoluteMaxPrice)}+</span>
          </div>
        </div>
      )}
